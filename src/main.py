...
async def save_memory(ctx: Context, text: str) -> str:
    ...
    The content will be processed and indexed for later retrieval through semantic search. Run get_all_memories to check if the memory has been saved.
...
async def delete_memory(ctx: Context, memory_id: str) -> str:
    ...
    This tool allows you to remove unwanted or outdated memories from long-term storage.
    Use this when you need to clean up incorrect information or remove sensitive data.
    ...
    try:
        mem0_client = ctx.request_context.lifespan_context.mem0_client
        mem0_client.delete(memory_id, user_id=DEFAULT_USER_ID)
...
async def delete_all_memories(ctx: Context) -> str:
    ...
    WARNING: This action is irreversible and will remove all stored memories.
    Use with caution as this will permanently delete all memory data.
    ...
    try:
        mem0_client = ctx.request_context.lifespan_context.mem0_client
        mem0_client.delete_all(user_id=DEFAULT_USER_ID)
        return "Successfully deleted all memories for the user"
    except Exception as e:
